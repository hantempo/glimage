...
def LoadTextures():
    ...
    rgba_pixels = np.fromstring(image, dtype=np.uint8)
    import _glimage
    pixels = np.empty(shape=(ix*iy*3), dtype=np.uint8)
    _glimage.rgba8_to_rgb8(ix, iy, rgba_pixels, pixels)

    # Create Texture
    glBindTexture(GL_TEXTURE_2D, glGenTextures(1))   # 2d texture (x and y size)

    glPixelStorei(GL_UNPACK_ALIGNMENT,1)
    glTexImage2D(GL_TEXTURE_2D, 0, 3, ix, iy, 0, GL_RGB, GL_UNSIGNED_BYTE, pixels.tostring())
